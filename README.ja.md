...
testコマンドは以下を行います:
1. `g++`でコードをコンパイル（`prepkit_config.yaml`で設定可能）
2. オプションの入力で実行ファイルを実行
3. 提供された場合、出力を期待される結果と比較
4. 明確なエラーメッセージで成功または失敗を報告
...
### ✅ 完全実装済み
- **C++プリプロセッサ**: インクルード解決、constexpr置換（int、float、double、bool、char）、コメント削除
- **C++ミニファイア**: コンパイル互換性を保持したサイズ最適化出力
- **C++テストランナー**: 設定サポート付きコンパイル、実行、出力検証
- **設定システム**: `prepkit_config.yaml`によるプロジェクトレベルのデフォルト
- **プロジェクトスキャフォールディング**: AtCoder、Codingame、Kaggle用のボイラープレート生成
- **包括的なテスト**: CLIテスト、エラーハンドリング、ビルド検証を含む45のテスト

### ⚠️ 既知の制限
- **複雑なConstexpr**: リテラル値のみサポート（例: `constexpr int X = 10`）、式は未対応（例: `constexpr int Y = X * 2`）
- **文字列Constexpr**: 基本的なリテラルの文字列定数置換をサポート
